...
class Generator(nn.Module):
    def __init__(
        ...
        self.ch = gf_dim
        self.l1 = nn.Linear(latent_dim, (self.bottom_width ** 2) * self.ch)
        self.block2 = GenBlock(
            self.ch, self.ch, activation=activation, upsample=True, n_classes=n_classes
        )
        ...
        self.c5 = nn.Conv2d(self.ch, out_ch, kernel_size=3, stride=1, padding=1)

    def forward(self, z):

        h = z.flatten(start_dim=1)

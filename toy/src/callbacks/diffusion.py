...
#         signs, logabsdet = torch.slogdet(hessian)
    ...
    def negative_entropy_gain_no_grad(D, X, n_max_prop, return_push_X=False):
        ...
        for i_prop in range(0, X.size(0), n_max_prop):
            ...
            neg[i_prop:i_prop +
                n_max_prop] = Diffusion.negative_entropy_gain(push_X_prop, X_prop).detach()
        if return_push_X:
            return neg, push_X.detach()
        return neg

    def __init__(self, init_distrib_sampler, init_log_prob, n_max_prop=1024):
        self.init_distrib_sampler = init_distrib_sampler
        self.init_log_prob = init_log_prob
        self.n_max_prop = n_max_prop
        self.Ds = []

    def sample_init(self, n):
        X = self.init_distrib_sampler(n)
        if len(X.shape) == 1:
            return X.view(-1, 1)
        return X
    ...
    def mc_entropy(self, X, ignore_init=False, return_X_transformed=False, try_true_init_entropy=False):
        ...
        if not ignore_init:
            if try_true_init_entropy:
                try:
                    ...
                    try:
                        entropy += self.init_log_prob(X).mean()
                    except:
                        raise Exception(
                            "no way to estimate 'init_distrib' entropy")
            else:
                try:
                    entropy += self.init_log_prob(X).mean()
                except:
        ...
        for i in range(len(self.Ds)):
            ...
            neg, push_X = self.negative_entropy_gain_no_grad(
                self.Ds[i], X, self.n_max_prop, True)
            entropy += neg.mean()
            X = push_X
    ...
    def log_prob_trace(self, X_trace, ignore_init=False, backward_order=True):
        ...
        _log_prob = 0.
        if not ignore_init:
            _log_prob = self.init_log_prob(X_trace[-1]).view(-1)
        for i in range(len(self.Ds)):
            n_xs = -i - 1
            X_trace[n_xs].requires_grad_(True)
            neg = self.negative_entropy_gain_no_grad(
                self.Ds[i], X_trace[n_xs], self.n_max_prop, False)
    ...
    def log_prob(self,
        ...
        assert method == 'grad_gd', "method {} not yet implemented".format(
            method)
        ...
        if method == 'grad_gd':
            ...
            for i in range(len(Ds)):
                ...
                while True:
                    ...
                    _lambdas = (
                        lr_base / torch.sqrt(prev_grad_norms + 1e-6)).view(-1, 1)
                    while True:
                        _new_Xs = _Xs_prev - _lambdas * _grad
                        curr_grad_norms = func_grad_norm(
                            Ds[ds_num], _new_Xs, _Xs_curr)
                    ...
                    final_grad_norms = func_grad_norm(
                        Ds[ds_num], _Xs_prev, _Xs_curr)
                    max_grad_norm_history.append(
                        final_grad_norms.max().detach().cpu())
                    ...
                    if len(mask) == 0:
                        if verbose:
                            ...
                            print('max grad diff: ', func_grad_norm(
                                Ds[ds_num], Xs_prev, Xs_curr).max())
                        break
                    if j > max_iter:
                        if verbose:
                    ...
                    if j > no_progress_stop:
                        # print("max_grad_norm_history", max_grad_norm_history)
            ...
            _log_prob = self.log_prob_trace(
                Xs_backward, ignore_init=ignore_init)
